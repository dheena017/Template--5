...
import { Languages, Settings2, Loader2, PlayCircle, HardDrive, Zap } from 'lucide-react';
...
const ConfigStep = ({
    ...
    return (
        <motion.div 
            ...
            className="config-step-container w-full max-w-4xl mx-auto Translation-conversion-card"
        >
            <div className="p-0">
                {/* File Info Header */}
                <div className="Translation-file-card-header">
                    <div className="Translation-icon-container">
                        <div className="Translation-icon-glow" style={{ backgroundColor: activeTool.color }} />
                        <Languages size={30} style={{ color: activeTool.color }} className="z-10" />
                    </div>
                    <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <h2 className="text-xl font-black text-white truncate" title={fileInfo.name}>{fileInfo.name}</h2>
                        <div className="flex items-center gap-3">
                            <span className="Translation-meta-badge">{fileInfo.size}</span>
                            <span className="Translation-meta-badge" style={{ color: activeTool.color }}>{fromFormat} Translation</span>
                        </div>
                    </div>
                ...
                >
                    {isProcessing ? (
                        <div className="w-full relative z-10">
                            ...
                            <div className="Translation-progress-bar-bg">
                                <div className="Translation-progress-bar-fill" style={{ width: `${progress}%` }} />
                            </div>
                        </div>
                    ) : (

...
import { Video, Settings2, Loader2, PlayCircle, HardDrive, Zap } from 'lucide-react';
...
const ConfigStep = ({
    ...
    return (
        <motion.div 
            ...
            className="config-step-container w-full max-w-4xl mx-auto video-conversion-card"
        >
            <div className="p-0">
                {/* File Info Header */}
                <div className="video-file-card-header">
                    <div className="video-icon-container">
                        <div className="video-icon-glow" style={{ backgroundColor: activeTool.color }} />
                        <Video size={30} style={{ color: activeTool.color }} className="z-10" />
                    </div>
                    <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <h2 className="text-xl font-black text-white truncate" title={fileInfo.name}>{fileInfo.name}</h2>
                        <div className="flex items-center gap-3">
                            <span className="video-meta-badge">{fileInfo.size}</span>
                            <span className="video-meta-badge" style={{ color: activeTool.color }}>{fromFormat} VIDEO</span>
                        </div>
                    </div>

...
import { PDF, Settings2, Loader2, FileText, HardDrive, Zap } from 'lucide-react';
...
const ConfigStep = ({
    ...
    return (
        <motion.div 
            ...
            className="config-step-container w-full max-w-4xl mx-auto PDF-conversion-card"
        >
            <div className="p-0">
                {/* File Info Header */}
                <div className="PDF-file-card-header">
                    <div className="PDF-icon-container">
                        <div className="PDF-icon-glow" style={{ backgroundColor: activeTool.color }} />
                        <PDF size={30} style={{ color: activeTool.color }} className="z-10" />
                    </div>
                    <div className="flex-1 min-w-0 flex flex-col gap-1">
                        <h2 className="text-xl font-black text-white truncate" title={fileInfo.name}>{fileInfo.name}</h2>
                        <div className="flex items-center gap-3">
                            <span className="PDF-meta-badge">{fileInfo.size}</span>
                            <span className="PDF-meta-badge" style={{ color: activeTool.color }}>{fromFormat} VIDEO</span>
                        </div>
                    </div>
                    ...
                    {isProcessing ? (
                        <div className="w-full relative z-10">
                            ...
                            <div className="PDF-progress-bar-bg">
                                <div className="PDF-progress-bar-fill" style={{ width: `${progress}%` }} />
                            </div>
                        </div>
                    ) : (

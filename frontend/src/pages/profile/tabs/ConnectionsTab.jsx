        ...
        <motion.section className="premium-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="section-head">
            ...
            <div className="connection-tools">
              ...
              <motion.button className="text-btn" whileHover={{ scale: 1.05 }}><CheckSquare size={14} /> Accept All</motion.button>
            </div>
          </div>
            ...
            {filteredConnections.map((conn, i) => (
              <motion.div key={conn.id} className={`connection-row premium-card ${conn.status}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.05 }} whileHover={{ x: 4 }}>
                ...
                <div className="conn-actions">
                  {conn.status === 'pending' ? (
                    <>
                      <motion.button className="accept-btn" whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }}>Accept</motion.button>
                      <motion.button className="decline-btn" whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }}>Decline</motion.button>
                    </>
                  ) : (
                    <motion.button className="message-btn" whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }}><MessageCircle size={14} /> Message</motion.button>
                  )}
                </div>
              </motion.div>
            ))}
          ...
          <div className="suggestions-list">
            {[{ name: 'David Park', role: 'ML Engineer', mutual: 8 }, { name: 'Nina Patel', role: 'UX Designer', mutual: 5 }, { name: 'Oliver Chen', role: 'DevOps', mutual: 12 }].map((sugg, i) => (
              <motion.div key={i} className="suggestion-item" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.05 }} whileHover={{ x: 4 }}>
                ...
                <motion.button className="add-btn" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>+</motion.button>
              </motion.div>
            ))}
          </div>
        </motion.section>

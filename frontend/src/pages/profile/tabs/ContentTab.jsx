...
import { AudioWaveform, Clapperboard, Heart, History, Music, PlayCircle, Star, TrendingUp, Video, Zap } from 'lucide-react'

const ContentTab = ({ projectsData }) => {
      ...
      <motion.section className="premium-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="section-head">
          ...
          <div className="portfolio-filter-row">
            ...
            <motion.button className="text-btn" whileHover={{ scale: 1.05 }}>+ New Project</motion.button>
          </div>
        </div>
          ...
          <div className="draft-list">
            {drafts.map((draft, i) => (
              <motion.div key={draft.id} className="draft-row" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.05 }} whileHover={{ x: 4 }}>
                ...
                <div className="progress-mini"><div className="progress-bar-mini" style={{ width: `${draft.progress}%` }}></div></div>
